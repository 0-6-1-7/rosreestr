...
CaptchaReloadingCounterMax = 5 # Максимум попыток
SaveCaptcha = False # сохранение капчи в файл

##--- end of header ---##------------------------------------------------------------##

def SecStr(t, short=False):
    s1 = f"{t}"[-1]
    if len(f"{t}") > 1: s2 = f"{t}"[-2]
    else: s2 = ""
    if s2 != "1":

        if s1 == "1": return f"{t} {'с.' if short else 'секунда'}"
        elif s1 == "2" or s1 == "3" or s1 == "4" : return f"{t} {'с.' if short else 'секунды'}"
        else: return f"{t} {'с.' if short else 'секунд'}"
    else: return f"{t} {'с.' if short else 'секунд'}"
##--- end of SecStr ---##------------------------------------------------------------##
...
def printProgressBar (iteration, total):
    ...
    if screen_width > 70:
        length = screen_width - 50
    else:
        length = screen_width - 20
    fill = '█'
    filledLength = int(length * iteration // total)
    bar = fill * filledLength + '-' * (length - filledLength)
    msg_short = f'\rПауза [{bar}] {SecStr(total - iteration, True)}' ##17
    msg_full = f'\rПауза между запросами [{bar}] осталось {SecStr(total - iteration)}'##45
    if screen_width > 70:
        print(msg_full, end = "\r")
        print(" " * (screen_width - 1 - len(msg_full)), end = "\r")
    else:
        print(msg_short, end = "\r")
        print(" " * (screen_width - 1 - len(msg_short)), end = "\r")
##    print(f'\rПауза между запросами {SecStr(total)} [{bar}] осталось {SecStr(total - iteration)}      ', end = "\r")
    if iteration == total: print(" " * screen_width)
...
## Версия для заказа выписок о переходах права - раскомментировать три следующих строки
