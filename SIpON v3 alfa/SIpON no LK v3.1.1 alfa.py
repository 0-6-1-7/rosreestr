...
##------------------------------------------------------------##
def RRinit():
    global RR, Status, RunFromIDLE, lbl_captcha, Status

    Status.set("Запускается Chrome. Дождитесь появления капчи."); AppWindow.update()
...
def RRgo():
    ...
    Status.set("...идёт обработка файла..."); AppWindow.update()
...
####################
    ...
    while kn != None:
        ...
        for l in range(0, len(tl)):
            if l == 4:
##                if tl[l] == "Не указано для этого ОН или ошибка": ws.cell(row = r, column = l + 1).value = ""; continue
    ...
    wb.save(wbName)
    Status.set(f"Всё готово! ---------- [ {time.strftime('%H:%M:%S', time.localtime())} ] "); AppWindow.update()
