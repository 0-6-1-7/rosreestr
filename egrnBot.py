...
RunFromIDLE = True  # True если скрипт запущен из IDLE. Нужен для headless
    ...
    if AuthStatus == "GotAuthKey":
        ...
        while True:
            ...
            if time.monotonic() - t0 > 60:
                logging.warning("За 60 секунд ключ не принят.")
                return ("AuthTimeOut")
...
def EGRNSearchPage():
    ...
    try:
        ...
        logging.info("Страница поиска готова.")
        return ("SearchPageOK")
    except:
...
def EGRNSearch():
    ...
    try:
        vapp = EGRN.find_element_by_css_selector("div.v-app")
        fKNs = vapp.find_element_by_xpath(".//input[contains(@class,'v-textfield-prompt')]")
        fReg = vapp.find_element_by_xpath(".//input[contains(@class,'v-filterselect-input')]")
        sButton = vapp.find_element_by_xpath(
        ...
        sButton.click()
        logging.info("Поиск запущен")
        t0 = time.monotonic()
    except:
        return ("SearchKNsError")

    EGRN.implicitly_wait(1)
    logging.info("Ожидание результата поиска")
    ...
    while True:
        ...
        try:
            NotificationWarningText = EGRN.find_element_by_class_name("v-Notification-warning").get_attribute(
                "innerText")
        ...
        if DataTable == None and NotificationWarning == None:
            sleep(1)
            if time.monotonic() - t0 > 60:
                logging.info("За 60 секунд не дождались нужной страницы. Вероятно, ошибка сайта.")
                return ("SearchKNsError")

        elif DataTable != None:
            sleep(1)
            t = EGRN.find_element_by_css_selector("div.blockSmall").get_attribute("innerText")
            logging.info(f"Результаты поиска получены. {t}")
            return ("SearchKNsOK")
...
def RecognizeCaptcha(c):
    ...
    global SaveCaptcha
    try:
        img_base64 = EGRN.execute_script(
...
def GetCaptcha(vapp):
    ...
    while True:
        ...
        CaptchaReloadButton = vapp.find_element_by_xpath(
...
def CheckRQfile():
    ...
    if RQStatus == None:
        ...
        for row in ws.iter_rows(min_row=3):
            if row[2].value == None:
                KNsToDo = KNsToDo + 1
            elif re.search(r"80-\d{9}", f"{row[2].value}") != None:
                NZcount = NZcount + 1
            elif row[2].value == "Не найден во ФГИС ЕГРН":
                ZZcount = ZZcount + 1
            else:
                print(
                    f"Произошло что-то странное со списком в строке {row.row - 2}")  ## мы не должны дойти до этого места, но вдруг...
        ...
        logging.info("Подготовка списка КН")
        for row in ws.iter_rows(min_row=3):
            if re.search(r"80-\d{9}", f"{row[2].value}") == None and row[2].value != "Не найден во ФГИС ЕГРН":
                BatchKNcount = BatchKNcount + 1
                if BatchKNcount <= BatchMax:
                    KNs = f"{KNs};{row[0].value}"  # добавим в список не больше этого количества КН
                else:
                    break
        if KNs[0] == ";":
            KNs = KNs[1:]

        logging.info(f"Всего кадастровых номеров {KNsToDo + NZcount + ZZcount}")
        if NZcount + ZZcount > 0:
            logging.info(f"Уже выполнены: {NZcount}, ещё не выполнены: {KNsToDo}")
            if ZZcount > 0:
                logging.info(f"Не найдены во ФГИС ЕГРН: {ZZcount}")
        if BatchKNcount < KNsToDo:
            logging.info(f"Обработка частями по {BatchMax} КН")
        RQStatus = "FileRQOK"
    return (RQStatus)
...
def GetInfo():
    ...
    while EGRN.find_element_by_css_selector("div.blockGrey").is_displayed():
        if time.monotonic() - t0 > 60:
            logging.warning("За 60 секунд не дождались нужной страницы. Вероятно, ошибка сайта.")
            return (9)

    KN = re.search(r"\b\d{2}:\d{2}:\d{1,7}:\d{1,}\b",
                   EGRN.find_element_by_css_selector("div.header3").get_attribute("innerText"))[0]
    logging.info(f"{KN} - подготовка запроса " + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    vapp = EGRN.find_element_by_css_selector("div.v-app")
    ...
    while True:
        ...
        if PopUpType == "Normal":
            OkButton = PopUp[0].find_elements_by_xpath(
                ".//span[contains(@class,'v-button-caption') and contains(text(),'Продолжить работу')]")[0]
            NZ = ""
            NZ = re.search(r"\d{2}\-\d{9}", PopUp[0].get_attribute("innerText"))[0]
            OkButton.click()
            logging.info(f"{KN} :: {NZ} - запрос выполнен в {PreviousRequestTimeStamp} "
                         f"за {round(PreviousRequestTimeMark - CurrentRequestTimeMark, 2)} сек.")
...
def GetAll(tout=defaultTimeout):
    ...
    for KN in AllKN:
        ...
        while True:
            ...
            logging.info(f"Объект {i} из {len(AllON)} ({KNsToDo})")
            time.sleep(2)
            try:
                ON.click()
            except:
                logging.warning(
                    f"Невозможно кликнуть на найденном объекте\nФатальная ошибка, придётся начинать сначала")
            ...
            if GetInfoStatus == 0:  # Запрос выполнен
                ...
                if PreviousRequestTimeMark != None:
                    SecondsElapsed = time.monotonic() - PreviousRequestTimeMark
                    if SecondsElapsed > tout:
                        Wait(tout)
                    else:
                        Wait(int(tout - SecondsElapsed - 25))
                else:
                    Wait(tout)
                break
            elif GetInfoStatus == 1:
                # Превышен интервал между запросами или ошибка регистрации запроса
                RetryCount = RetryCount + 1
                logging.info(f"Попытка № {RetryCount + 1}")

            elif GetInfoStatus == 9:
                logging.info("Фатальная ошибка, придётся начинать сначала")
                send_email("EGRN bot FATAL ERROR", "Fatal error! Restart.")
                return "FatalError"
...
while True:
    ...
    if Status == "FileRQOK":
        while True:
            logging.info("Начало работы с сайтом")
            Status = EGRNinit()
            ...
            if Status == "SearchPageOK":
                Status = EGRNSearch()
            if Status == "SearchKNsOK":
                Status = GetAll()

            if Status == "AllDone":
                break
            if Status == "BatchDone":
                logging.info("Перезапуск для обработки очередной части списка")
                break
            if Status == "SearchKNsNotFound":
                logging.warning("Ни один из запрошенных КН не найден во ФГИС ЕГРН.")
                MarkKNAsNotFound()
                logging.info("Перезапуск для обработки очередной части списка")
                break
            else:
                logging.warning("Перезапуск после фатальной ошибки");
                break
    else:
        time.sleep(3)

    if Status == "FileRQAllDone":
        break
    if Status == "AllDone":
        break
