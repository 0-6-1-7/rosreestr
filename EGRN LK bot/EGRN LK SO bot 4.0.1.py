...
################################################################################
################################################################################
            menu = RR.find_element(By.CSS_SELECTOR, "div.rros-ui-lib-dropdown-menu > div > button")
            menu.click()
            sleep(1)
            menu_item = RR.find_element(By.CSS_SELECTOR, "div.rros-ui-lib-dropdown-menu__menu-el:nth-child(0) > div > div > div")
            menu_item.click()
################################################################################
################################################################################

            # ожидание результата отправки
            # ожидание может быто долгим, особенно в случе ошибки
            t0 = monotonic()
            while True:
                if monotonic() - t0 > 120:
                    try:
...
def check_notifications(RR):
    ...
    notifications_text = errors.get_attribute("innerText")
