from get_info_2 import get_info_kn, get_info_addr, get_header
...
def wb_save_30_sec(wb, wbName, save_time_mark):
    if time.monotonic() - save_time_mark > 60:
        wb.save(wbName)
        print("saved")
...
## обработка порциями по несколько строк
            ...
            range_of_KNs = ws[range_name(row, 2, row + Multi - 1, len(header) + 2)]
...
## на выходе либо ошибка
            if t[:5] == "ERROR": save_time_mark = wb.save(wbName); print_status(t); return
            if t[:5] == "FATAL": save_time_mark = wb.save(wbName); print_status(t); return
...
## добавить заголовок, если в файле его нет
    if rows_done < 0:
        header = "ЛС (может быть пустым)\t"\
            "Адрес\t"\
            "Найдено кадастровых номеров".split("\t")
        for i in range(0, len(header)): ws.cell(row=1, column=i + 1).value = header[i]
...
## лист с результататми текущего поиска
    ...
    header = ("ЛС\t"\
        ...
        "Кадастровый номер\t"\
        + get_header()).split("\t")
    for i in range(0, len(header)): ws2.cell(row=1, column=i + 1).value = header[i]
...
## нужно иметь это в виду
                    ...
                    for col in range(1, 4 + len(header)):
