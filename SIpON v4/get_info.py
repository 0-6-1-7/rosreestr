...
def captcha_OK(RR):
    try:
        RR.find_element(By.CSS_SELECTOR, "span.rros-ui-lib-message--error")
        return "FATAL_ERROR_CAPTCHA_WRONG"
    except NoSuchElementException: return "OK"
    except InvalidSessionIdException: return "FATAL_ERROR_CHROME"
    else: return "FATAL_ERROR_CAPTCHA_FAIL"
...
def get_info_kn(RR, D):
    try: #wrap for any return
        ...
        if kn_input.get_attribute("disabled"): return "FATAL_ERROR_CAPTCHA_NOT_ENTERED"
        ...
        captcha_status = captcha_OK(RR)
        if captcha_status != "OK" : return captcha_status
...
def get_info_addr(RR, data, D):
    try: #wrap for any return
        ...
        if data_input.get_attribute("disabled"): return "FATAL_ERROR_CAPTCHA_NOT_ENTERED"
        n = 1
        while data_input.get_attribute("value") != "":
            try:
                data_input.send_keys(Keys.ENTER)
                data_inputClear = wait.until(
                    EC.visibility_of_element_located(
                        (By.CSS_SELECTOR, "div.rros-ui-lib-dropdown__clear-indicator")))
                data_inputClear.click()
            except:
