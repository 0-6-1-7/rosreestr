from get_info_6 import get_info_kn, get_info_addr, get_header
...
from time import monotonic, sleep, strftime, gmtime
from tkinter import *
...
def get_file_to_proceed():
    ...
    print_progress(total=PROGRESS[0], done=PROGRESS[1], message=PROGRESS[2], win_update=True)

    if PROGRESS[:2] != [None, None]:
...
## посчитать обработанные строки
    ## строка считается не обработанной, если не указана дата обновления (столбец D)
    ## таким образом будут посчитаны и пустые строки, и строки с ошибками
    rows_empty = 0
    rows_done = 0
    for row in range(2, ws.max_row + 1):
        if ws.cell(row=row, column=4).value is None: rows_empty += 1
    rows_done = rows_total - rows_empty

    if rows_empty == 0:
        return [rows_total, rows_done, "Файл полностью обработан"]

## в файле есть данные, определить тип файла - адреса или кадастровые номера
    if match(r"^\d{1,2}:\d{1,2}:\d{6,7}:\d{1,10}$", ws.cell(row=2, column=2).value) is None:
        SEARCH_TYPE = "addr"
    else:
        SEARCH_TYPE = "KN"
    return [rows_total, rows_done, None]
...
def print_progress(total=0, done=0, done_batch=0, duration=0, message=None, win_update=True):
    ...
    if done_batch > 0:
        msg = msg + f"\nв т.ч. {done_batch}" + \
            (f" за {strftime('%H:%M:%S', gmtime(duration))} ({int(duration)} сек.)" if duration > 0 else "") + \
...
def print_status(msg, win_update=True):
    ...
    if msg[:5] == "FATAL":
        ...
        if msg == "FATAL_LIMIT":
            txt = "Произошла ошибка получения сведений об объектах недвижимости. Пожалуйста, повторите свой запрос позже."
        if msg == "FATAL_OTHER_OBJECT":
            txt = "Открылась карточка от друго кадастрового номера"
        ...
        if msg == "WARNI_SESSION_ENDED_REFRESH_PAGE":
            txt = "Время сессии истекло. Пожалуйста, обновите страницу"
        if msg == "WARNI_STATUS_500":
            txt = "Unsuccessful status come from fgis egrn request client 500"
        if msg == "WARNI_STATUS_502":
            txt = "Request failed with status code 502"
    elif msg[:5] == "NOTIF":
        txt = msg.split("_")[1]
    else: txt = msg
    print(txt)
...
# значение таймаута
    ...
    lbl_about = Label(APP_WINDOW, text="Поиск по адресам и кадастровым номерам :: версия 5.201")
        ...
        if "esia.gosuslugi.ru/login/" in current_url:
            try:
                ...
                if esia_login is not None:
                    form_login.send_keys(Keys.CONTROL + "a")
                    form_login.send_keys(Keys.DELETE)
                    form_login.send_keys(esia_login)
                if esia_password is not None:
                    form_password.send_keys(Keys.CONTROL + "a")
                    form_password.send_keys(Keys.DELETE)
...
##        chrome_options.add_argument("user-data-dir=" + getenv("TEMP") + "\\rosreestr")
...
def RRsteady():
    ...
    while True:
        ...
        if monotonic() - t0 > AUTH_TIMEOUT:
            ...
            return "FATAL_ERROR_INIT_START_PAGE_NOT_LOADED"
...
# ------------------------------------------------------------ #
def RRgo():
    RRsteady() # переход на нужную страницу 
    ...
    while True:
        ...
        if return_status in "STOP":
            break
        if return_status in "WARNI;ERROR":
            RR.refresh()
            if RRsteady() == "OK":
                continue
            else:
...
## Проверить и добавить заголовки столбцов начингая с C
...
## основной цикл обработки
        ...
        t1 = 0 # точка отсчёта таймаута для первого запроса
...
## эта строка уже обработана (указана дата обновления информации)
            ...
            if match(r"^\d{1,2}:\d{1,2}:\d{6,7}:\d{1,10}$", kn) is None:
                print(f"kn = {kn} - это не кадастровый номер")
                ws.cell(row=row, column=3).value = "это не кадастровый номер"
                ws.cell(row=row, column=4).value = "ошибка"
                row = row + 1
                continue

            print("\n\nНачало обработки кадастрового номера")
            print(f"kn = {kn} - подготовка запроса")
            print_status(f"Обработка кадастрового номера {kn}")
##            wait_timeout(t1, 1) # а нужен ли вообще таймаут в это месте?
...
## обработка результата выполнения запроса 
    ## запрос завершился с ошибкой
            t1 = monotonic() # время для следующего таймаута (будет использоваться только в случае ошибки)
            if type(result) == type("string"):
                print("==================")
                print(result)
                print("==================")
                object_info = result # сведения из карточки объекта
            else:
            ...
            if object_info is not None:
                if object_info[:5] in "FATAL":
                    wb.save(wbName)
                    return "STOP"
                if object_info[:5] in "NOTIF;ERROR":
                    print_status(f"")
                    sleep(5)
                    retry_count += 1
                    if retry_count < 5:
                        if WORK_STOPPED:
                            wb.save(wbName)
                            return "STOP"
                        print_status(f"\tПопытка повторить поиск {retry_count}")
                        continue
                    else:
                        return "ERROR"

    ## запрос завершился успешно, разделитель \t
            t1 = result['last_query_time'] # время успешного окончания обработки запроса для следующего таймаута
            retry_count = 0 # сросить счётчик попыток
            t = object_info.split("\t")
            for i in range(0, len(t)):
##                if i == 3: # площадь - может быть записана как через ".", так и через ","
##                    if t[i]:
##                        ws.cell(row=row, column=i + 3).value = float(t[i].replace(",", "."))
##                else: # остальные поля не нужаются в преобразовании
...
##########################################################################
RUN_FROM_IDLE = "idlelib" in modules
main()
