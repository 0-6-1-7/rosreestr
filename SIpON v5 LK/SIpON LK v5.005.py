...
def get_with_ESIA_auth(RR, URL):
    ...
    try:
        ...
        auth_login = auth.get("ESIA", "login")
        auth_password = auth.get("ESIA", "password")
        auth_name = auth.get("ESIA", "name")
    except:
        return "ERROR_CANT_READ_ESIA_FILE"
    ...
    try:
        ...
        return "ERROR_AUTH_PAGE_HAS_NO_FORM"

    form_login.send_keys(auth_login)
    form_password.send_keys(auth_password)
    form_button.click()
    sleep(1)
    ...
    try:
        ...
        return "INFO_NO_PERSONAL_CABINET"

    try:
        roles_selector = personal_cabinet_root.find_element(
            By.CSS_SELECTOR, "div.roles-selector")
        role = roles_selector.find_element(
            By.XPATH, "//*[contains(text(),'" + auth_name + "')]")
        role.click()
    except:
        sleep(1)
