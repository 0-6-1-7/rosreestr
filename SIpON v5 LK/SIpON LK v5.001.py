...
##            range_of_kn = ws[range_name(row, 2, row, len(header) + 2)]
...
## на выходе либо ошибка
            if object_info[:5] == "ERROR":
                save_time_mark = wb.save(wbName); print_status(object_info); return
            if object_info[:5] == "FATAL":
                save_time_mark = wb.save(wbName); print_status(object_info); return

## либо строка с результатами, разделитель \t
            if object_info is None: r[1].value = "Объект по кадастровому номеру не найден"; continue
