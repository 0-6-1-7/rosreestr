...
# defaultTimeout = 60  ## пауза между запросами в сеукндах 315
...
RunFromIDLE = False  # True если скрипт запущен из IDLE. Нужен для headless
...
# True для заказа выписок о переходах права (как в EGRN bot), False для заказа выписок о сведениях об объекте  (как в EGRN bot SO, как раньше)
SO = False
DEBUG = True
if DEBUG:
    logging.basicConfig(
...
def CheckRQfile():
    ...
    if RQStatus == None:
        ...
        if NZcount + ZZcount > 0:
            logging.info(f", в том числе:\n\tуже выполнены: {NZcount}, ещё не выполнены: {KNsToDo}", end="")
            if ZZcount > 0:
                logging.info(f", не найдены во ФГИС ЕГРН: {ZZcount}", end="")
        if BatchKNcount < KNsToDo:
            logging.info(f", обработка частями по {BatchMax} КН", end="")
        RQStatus = "FileRQOK"
    return (RQStatus)
...
def GetInfo():
    ...
    while True:
        ...
        if PopUpType == "Normal":
            ...
            OkButton.click()
            print(f"{KN} :: {NZ} - запрос выполнен в {PreviousRequestTimeStamp} "
...
def GetAll(tout=defaultTimeout):
    ...
    for KN in AllKN:
        ...
        while True:
            ...
            logging.info(f"Объект {i} из {len(AllON)} ({KNsToDo}) :: ", end="")
            time.sleep(2)
            try:
                ON.click()
            except:
                logging.warning(f"Невозможно кликнуть на найденном объекте\nФатальная ошибка, придётся начинать сначала")
