...
def EGRNSearch():
    ...
    try:
        ...
        sButton = vapp.find_element_by_xpath(".//span[contains(@class,'v-button-caption') and contains(text(),'Найти')]")
    ...
    while True:
        DataTable = None
        try: DataTable = EGRN.find_element_by_css_selector("table.v-table-table")
        except: pass
        NotificationWarning = None
        try:
            NotificationWarningText = EGRN.find_element_by_class_name("v-Notification-warning").get_attribute("innerText")
...
##    print("Обработка капчи")
    ...
    while True:
        time.sleep(1)
        try: CaptchaImage = vapp.find_element_by_xpath(".//img[@style='width: 180px; height: 50px;']")
...
## Версия для заказа выписок о сведениях об объекте
