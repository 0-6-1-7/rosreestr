...
def print_status(msg, win_update=True):
    ...
    if msg[:5] == "FATAL":
        ...
        if msg == "ESIA_ERROR_TIMOUT":
            txt = "Не удалось авторизоваться. Попробуйте ещё раз вручную."
    else: txt = msg
    print(txt)
    STATUS.set(txt)
    if win_update: APP_WINDOW.update()
...
def RRgo():
    ...
    while True:
        ...
        print_status(f"Ожидание загрузки начальной страницы, прошло {floor(monotonic() - t0)} сек. из {AUTH_TIMEOUT}")
        sleep(1)
...
## основной цикл обработки
        while row <= ws.max_row:
            ...
            print_status("Поиск объекта, подготовка запроса")
...
## отработать таймаут 5 секунд
            ...
            while True:
                if monotonic() - t0 > AUTH_TIMEOUT:
                    print_status("Ошибка: поиск не завершился за отведённое время")
                    return "ERROR"
                try:
...
## отработать таймаут 5 секунд
            t0 = monotonic()
            while monotonic() - timeout_counter < 6:
                    sleep(1)
                    print("sleep(1)")
            timeout_counter = monotonic()
            ...
            while True:
                if monotonic() - t0 > AUTH_TIMEOUT:
                    print_status("Ошибка: объект найден, но не отображается окно с информацией об объекте")
                    return "ERROR"
                try:
...
################################################################################
            ...
            menu_item = RR.find_element(By.CSS_SELECTOR,
                "div.rros-ui-lib-dropdown-menu__menu-el:nth-child(1) > div > div > div")
            menu_item.click()
################################################################################
################################################################################
            ...
            while True:
                ...
                try:
                    ...
                    None
                print_status(f"Ожидание результата отправки запроса, прошло {floor(monotonic() - t0)} сек. из {AUTH_TIMEOUT}")
                sleep(1)
