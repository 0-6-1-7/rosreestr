...
def SIpONinit():
    ...
    while True:
        ...
        try: SIpON.get("https://rosreestr.ru/wps/portal/online_request")
        except: SIpON.execute_script("window.stop();")

        try: form = SIpON.find_element_by_id("online_request_search_form_span")
        except: plt = plt + 10

        if form != None: return 0
        if plt > 30: plt = 10; SIpONrestart()
#### end of SIpONinit ##------------------------------------------------------------##

def SIpONrestart():
    global SIpON
    if SIpON != None: SIpON.close(); SIpON.quit()
    print(f"Restart")
    SIpONinit()
#### end of SIpONrestart ##------------------------------------------------------------##
...
def GetInfoByAddr(new_subject = "", new_region = "", new_settlement = "", new_street = "", new_house = "", new_apartment = ""):
    global SIpON
    prev_subject = None; prev_region = None; prev_settlement = None; prev_street = None; prev_house = None; prev_apartment = None

    Search_retry_count = 0 # номер попытки
    status = None
    prev_links = None

    SIpON.implicitly_wait(1); SIpON.set_page_load_timeout(10)
    ...
    while True: # цикл крутится до получения результата или 10 попыток
        ...
        try: SIpON.find_element_by_css_selector("a[href*='SetSearchCritVisible']").click()
        except: pass
        ...
        while True: # цикл крутится пока не будет заполнена форма или 10 попыток
            ...
            if Fill_retry_count > 3: status = 999; break 
            else: time.sleep(3)

            #№ пытаемся получить содержимое формы
            ## префикс f_ - ссылка на элемент DOM; prev_ - текущее значение поля; new_ - новое значение для поиска
            try: f_subject = SIpON.find_element_by_css_selector("select[name='subject_id']"); prev_subject = Select(f_subject).first_selected_option.text
            except: Fill_retry_count = Fill_retry_count + 1; continue # субъект не может быть пустым, повторить цикл
            
            try: f_region = SIpON.find_element_by_css_selector("select[name='region_id']"); prev_region = Select(f_region).first_selected_option.text
            except: prev_region = None  #  регион может быть пустым
            
            try: f_settlement = SIpON.find_element_by_css_selector("select[name='settlement_id']"); prev_settlement = Select(f_settlement).first_selected_option.text
            except: prev_settlement = None #  нас. пункт может быть пустым

            try:
                f_street = SIpON.find_element_by_css_selector("input[name='street']"); prev_street = f_street.get_attribute('value')
                f_house = SIpON.find_element_by_css_selector("input[name='house']"); prev_house = f_house.get_attribute('value')
                f_apartment = SIpON.find_element_by_css_selector("input[name='apartment']"); prev_apartment = f_apartment.get_attribute('value')
            except: Fill_retry_count = Fill_retry_count + 1; continue
            ...
            if ifNone(prev_subject) != ifNone(new_subject):
                try: f_subject.send_keys(Keys.NULL); f_subject.send_keys(Keys.HOME); f_subject.send_keys(new_subject); time.sleep(3)
                except: Fill_retry_count = Fill_retry_count + 1; continue 

            # регион 
            if ifNone(prev_region) != ifNone(new_region):
                try: f_region.send_keys(Keys.NULL); f_region.send_keys(Keys.HOME); f_region.send_keys(new_region); time.sleep(3)
                except: Fill_retry_count = Fill_retry_count + 1; continue 
            ...
            if ifNone(prev_settlement) != ifNone(new_settlement):
                if f_settlement.get_attribute("disabled"): # если поле не заполнено
                    if new_settlement != None: Fill_retry_count = Fill_retry_count + 1; continue # а должно быть
                else: # поле заполнено
                    f_settlement.send_keys(Keys.HOME) # непустой НП или очистка поля
                    if new_settlement != None: f_settlement.send_keys(new_settlement) # непустой НП

            try:
                if ifNone(prev_street) != ifNone(new_street):
                    f_street.click(); f_street.clear()
                    if new_street != None: f_street.send_keys(new_street)
                if ifNone(prev_house) != ifNone(new_house):
                    f_house.click(); f_house.clear()
                    if new_house != None: f_house.send_keys(new_house)
                if ifNone(prev_apartment) != ifNone(new_apartment):
                    f_apartment.click(); f_apartment.clear()
                    if new_apartment != None: f_apartment.send_keys(new_apartment)
            except: Fill_retry_count = Fill_retry_count + 1; continue
            
            #№ проверка заполнения формы

            try: prev_subject = Select(f_subject).first_selected_option.text
            except: pass
            
            try: prev_region = Select(f_region).first_selected_option.text
            except: prev_region = None
            
            try: prev_settlement = Select(f_settlement).first_selected_option.text
            except: prev_settlement = None
            ...
            if TestFilledData:
                ...
                Fill_retry_count = Fill_retry_count + 1; continue
        ...
        except: Search_retry_count = Search_retry_count + 1; continue # не найдена кнопка поиска? 
    
        ## ожидание результата поиска
        if status != 0: Search_retry_count = Search_retry_count + 1; continue
        Result_wait_count = 0
        while True:
            if Result_wait_count > 4: status = 999; break
            elif Result_wait_count > 0: time.sleep(5)
            else: time.sleep(1)
            try:
...
else: wbName = (os.path.dirname(sys.argv[0]) + "/KNaddr.xlsx").replace("/", "\\")
