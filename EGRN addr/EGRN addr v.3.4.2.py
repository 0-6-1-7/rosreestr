...
def field_filterselect(field, field_data, wait_while_disabled=False):
    ...
    except: return "FATAL_ERROR_DATA_NOT_FOUND"

    while True:
        suggest = suggestpopup.find_element(By.CSS_SELECTOR, "tr")
        suggest_text = suggest.get_attribute("innerText")
        if field_data and field_data.upper() in suggest_text.upper():
            break
        if not field_data and '\xa0' in suggest_text:
            break
        time.sleep(1)
...
def EGRN_search(data):
    ...
    try:
        ...
        try:
            ...
            if f4_d != PREV_DATA[3]:
                sta = field_filterselect(f4, f4_d, wait_while_disabled=True)
